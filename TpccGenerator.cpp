...
void TpccGenerator::generateDistricts() {
  ...
  for (d_w_id = 1; d_w_id <= warehouse_count; d_w_id++) {
    for (d_id = 1; d_id <= kDistrictsPerWarehouse; d_id++) {
      ...
      makeAddress(d_street_1.data(), d_street_2.data(), d_city.data(),
                  d_state.data(), d_zip.data());
      d_tax = ((float)makeNumber(10L, 20L)) / 100.0f;

      // @formatter:off
      d_csv << d_id << d_w_id << d_name << d_street_1 << d_street_2 << d_city
            << d_state << d_zip << csv::Precision(4) << d_tax
            << csv::Precision(2) << d_ytd << d_next_o_id << csv::endl;
      // @formatter:on
    }
  }
...
void TpccGenerator::generateCustomerAndHistory() {
  ...
  array<char, 22> c_since =
      {}; // XXX used in history and customer and generated over and over again
  ...
  for (c_w_id = 1; c_w_id <= warehouse_count; c_w_id++) {
    for (c_d_id = 1; c_d_id <= kDistrictsPerWarehouse; c_d_id++) {
      for (c_id = 1; c_id <= kCustomerPerDistrict; c_id++) {
        ...
        makeAddress(c_street_1.data(), c_street_2.data(), c_city.data(),
                    c_state.data(), c_zip.data());
        ...
        // @formatter:off
        c_csv << c_id << c_d_id << c_w_id << c_first << c_middle << c_last
              << c_street_1 << c_street_2 << c_city << c_state << c_zip
              << c_phone << c_since << c_credit << csv::Precision(2)
              << c_credit_lim << csv::Precision(4) << c_discount
              << csv::Precision(2) << c_balance << 10.0f << int64_t(1)
        ...
        // @formatter:off
        h_csv << cnt << c_id << c_d_id << c_w_id << c_d_id << c_w_id << c_since
              << h_amount << h_data << csv::endl;
        // @formatter:on
      }
    }
  }
...
void TpccGenerator::generateItems() {
  ...
  for (i_id = 1; i_id <= kItemCount; i_id++) {
    ...
    // @formatter:off
    i_csv << i_id << i_im_id << i_name << csv::Precision(2) << i_price << i_data
          << csv::endl;
    // @formatter:on
  }
...
void TpccGenerator::generateStock() {
  ...
  for (s_w_id = 1; s_w_id <= warehouse_count; s_w_id++) {
    ...
    for (s_i_id = 1; s_i_id <= kItemCount; s_i_id++) {
      ...
      // @formatter:off
      s_csv << s_i_id << s_w_id << s_quantity << s_dist_01 << s_dist_02
...
void TpccGenerator::generateOrdersAndOrderLines() {
  ...
  int64_t o_ol_cnt;
  array<char, 22> o_entry_d = {}; // XXX not sure if date is generate correctly
...
uint32_t TpccGenerator::makeAlphaString(uint32_t min, uint32_t max,
                                        char *dest) {
...
uint32_t TpccGenerator::makeNumberString(uint32_t min, uint32_t max,
                                         char *dest) {
...
void TpccGenerator::makeAddress(char *street1, char *street2, char *city,
                                char *state, char *zip) {
...
uint32_t TpccGenerator::makeNonUniformRandom(uint32_t A, uint32_t x,
                                             uint32_t y) {
...
void TpccGenerator::makeNow(char *str) {
  ...
  string ymd = to_string(1900 + ltm->tm_year) + "-" + to_string(1 + ltm->tm_mon) + "-" + to_string(ltm->tm_mday);
  string hms = to_string(ltm->tm_hour) + ":" + to_string(ltm->tm_min) + ":" + to_string(ltm->tm_sec);
  string s = ymd + " " + hms + "+07";
